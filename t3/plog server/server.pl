...
parse_input(init_board,Board):-
	...
	parse_input(possible_moves(Board,Piece),List):-
		find_pos(Board, Piece, Pos),
		possible_moves(Pos,List1),
		findall(X,(member(X,List1),get_piece_between(Board, Piece, X, CapturedPiece, CapturedPiecePos)),List).
