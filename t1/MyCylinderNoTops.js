	...
	var teta=2*Math.PI/this.slices;
 	for(var j=0;j<=this.stacks;j++){
 		...
 		for(var i=0;i<this.slices;i++){
 			this.vertices.push(delta * Math.cos(i*teta));      //push the vertices and normals
 ...
 this.indices=[];
 	...
 	for(var j=0;j<this.stacks;j++){
 		for(var i=0;i<this.slices;i++){
			...
			this.indices.push(j*this.slices+(i+1)%this.slices);            //push the indices
     ...
     var s = 0;
	var t = 0;
	var s_inc = 1/this.slices;                   //builds the texture
